...
datos <- read.csv(fname,stringsAsFactors=F, sep = "\t")
library(hms)
str(datos)
datos$FECHA <- as.POSIXlt(datos$FECHA, format="%d/%m/%Y")
datos$HORA <- as_hms(datos$HORA)
str(datos) # vemos que FECHA y HORA tienen los formatos deseados
save(datos, file="data/datos_ejemplo.RData")
```
...
library(readr)
BreadBasket_DMS <- read_csv(filename)
#View(BreadBasket_DMS)
save(BreadBasket_DMS,file="data/BreadBasket_DMS.RData")
```
...
psych.bike <- read_sav(filename)
save(psych.bike,file = "data/PsychBike.RData")
```
...
file_json <- read_json(filename,simplifyVector = F)
class(file_json) # si no simplificamos, la información acaba siendo una lista de listas (engorroso)
file_json_simplified <- read_json(filename, simplifyVector = T) # si simplificamos es un dataframe 
class(file_json_simplified)
save(file_json_simplified,file="data/FileCodificado.RData")
```
...
```{r}
# str, head, tail, summary
str(file_json_simplified)
head(file_json_simplified)
tail(file_json_simplified)
summary(file_json_simplified)
```
...
```{r}
write_json(iris, "data/iris.json")
iris_json <- read_json("data/iris.json",simplifyVector = T)
all(iris == iris_json) # son iguales
```
...
# copiamos la primera fila para sacar los nombres de las columnas
primera_fila <- "IDENTIFICA	RAZA	FECHA NAC	SEXO	FECHA ENTRADA	FECHA SALIDA	RESULTADO	ENF BASE	COMOR	FECHA CONTROL	EPO	DOSIS EPO	EQUIVALENCIA epo MES	HTA	tto hta	HVI	ICC	VHC	TABACO	DISLIPEMIA	COL TOT	COL HDL	COL LDL	TG	DM	VIT D	HIDROF	ANTIAG	ANTICOAG	HIERRO	BICAR	DIURET	IECAS	PESO	TALLA	IMC	HB	IST 	FERRITINA	UREA	CREAT 	FOSFORO	CALCIO	CICALCET	QUELANTES	D3	PTH 	BICARBO	ALOPURIN	URICO	PROTEINU 0	ALBUMI 0	sodio	potasio	CLORO	HB GLIC	GLUCOSA	PCR"
primera_fila <- strsplit(primera_fila,split = "\t")
primera_fila <- primera_fila[[1]] # nos quedamos con la lista de nombres

col_numeric <- "COL*, TG, HIDROF, ANTI*, DIURET, IECAS, PESO, TALLA?, HB, IST, FERRITINA, UREA, FOSFORO, CALCIO, CICALCET, QUELANTES, PTH, BICARBO, ALOPURIN, URICO, PROTEINU*, ALBUMI*, sodio, potasio, cloro, HB GLIC, GLUCOSA, PCR"
col_numeric <- gsub("[[:punct:]]","",col_numeric)
col_numeric <- strsplit(col_numeric,split = " ")[[1]]

clasifica <- function(nombre){
  if(nombre %in% col_numeric){
    return("numeric")
...
ct <- sapply(primera_fila,clasifica)

df <- read_excel(fname, sheet=2, col_types=ct)

# guardamos datos en formato RData
save(df,file = "data/ERCA.RData")
```
